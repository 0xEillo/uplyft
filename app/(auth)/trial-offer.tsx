...
export default function TrialOfferScreen() {
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      if (user && params.onboarding_data) {
        const onboardingData = JSON.parse(params.onboarding_data as string)
        await database.profiles.update(user.id, {
          display_name: onboardingData.name,
          gender: onboardingData.gender,
          height_cm: onboardingData.height_cm
            ? parseFloat(onboardingData.height_cm)
            : null,
          weight_kg: onboardingData.weight_kg
            ? parseFloat(onboardingData.weight_kg)
            : null,
          age: onboardingData.birth_year
            ? new Date().getFullYear() - parseInt(onboardingData.birth_year)
            : null,
      ...
      Alert.alert(
        ...
        'Please try again or continue without a trial.',
        [{ text: 'OK' }],
      )
    } finally {
