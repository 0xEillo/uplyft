...
export default function TrialOfferScreen() {
  ...
  const dynamicTitle = useMemo(() => {
    if (!onboardingData || !onboardingData.goal || onboardingData.goal.length === 0) {
      return "Get full access to your personalized plan!";
    }
    
    const goals = onboardingData.goal;
    if (goals.includes('build_muscle')) return "Get full access to your muscle building plan!";
    if (goals.includes('lose_fat')) return "Get full access to your weight loss plan!";
    if (goals.includes('gain_strength')) return "Get full access to your strength plan!";
    if (goals.includes('improve_cardio')) return "Get full access to your cardio plan!";
    if (goals.includes('become_flexible')) return "Get full access to your flexibility plan!";
  ...
  const handleStartTrial = async () => {
    try {
      ...
      if (!hasProEntitlement) {
        Alert.alert('Subscription Pending', "Purchase successful but taking a moment to activate.", [{ text: 'OK' }])
      }
                 ...
                 return (
                   ...
                   >
                      ...
                      <View style={[styles.cardInner, (isBestValue || isPopular) && styles.cardInnerWithBadge]}>
                          ...
                          <Text style={styles.planPrice}>{plan.package?.product.priceString || plan.price}</Text>
