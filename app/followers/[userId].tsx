...
export default function FollowersScreen() {
          ...
          if (
            ...
            setRelationships((prev) => ({
              ...
              [targetUser.id]: {
                ...
                target_id: targetUser.id,
                is_following: false,
                is_private: prev[targetUser.id]?.is_private ?? true, // Assuming private if pending
                has_incoming_request:
                  prev[targetUser.id]?.has_incoming_request ?? false,
                incoming_request_id:
                  prev[targetUser.id]?.incoming_request_id ?? null,
  ...
  const filteredFollowers = followers.filter((item) => {
    ...
    return (
      item.follower.display_name?.toLowerCase().includes(query) ||
      item.follower.user_tag?.toLowerCase().includes(query)
    )
  })
  ...
  const renderItem = ({ item }: { item: FollowerItem }) => (
    <UserListItem
      user={item.follower}
      relationship={currentUser ? relationships[item.follower.id] : undefined}
      onFollowAction={handleFollowAction}
      isOwnProfile={currentUser?.id === item.follower.id}
    />
  )
