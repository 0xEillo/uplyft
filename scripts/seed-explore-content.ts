...
function findExerciseId(name: string): string {
  ...
  const normalized = exercises.find(
    (e: any) =>
  ...
  for (const [key, val] of Object.entries(KEYWORDS)) {
    if (name.toLowerCase().includes(key)) {
      const match = exercises.find((e: any) =>
...
const PPL_PROGRAM = {
  name: 'Push Pull Legs',
  description:
    'A 3-day split (Push, Pull, Legs) focused on high intensity. Volume is lower to allow for maximum effort; each set should be performed near failure (0-2 reps in reserve).',
  level: 'beginner',
  goal: 'build_muscle',
  is_published: true,
  display_order: 1,
...
const PPL_ROUTINES = [
  {
    name: 'Push',
    description:
      'Upper body pushing muscles. Maintain high intensity and push each set near failure.',
    level: 'beginner',
    duration_minutes: 45,
  ...
  },
  {
    name: 'Pull',
    description:
      'Upper body pulling muscles. Low volume, high intensity is key—take every set to near failure.',
    level: 'beginner',
    duration_minutes: 45,
  ...
  },
  {
    name: 'Legs',
    description:
      'Lower body session. Focus on heavy compound movements and high intensity sets near failure.',
    level: 'beginner',
    duration_minutes: 45,
    equipment: ['Machine'],
    is_published: true,
...
const UL_PROGRAM = {
  ...
  description:
    'A 4-day split alternating between upper and lower body. Optimized for high intensity and lower volume; ensure every set is performed near failure for maximum growth.',
  level: 'intermediate',
  goal: 'build_muscle',
  is_published: true,
  display_order: 2,
...
const UL_ROUTINES = [
  {
    name: 'Upper Body',
    description:
      'Focuses on compound upper body pushing and pulling movements.',
    level: 'intermediate',
    duration_minutes: 60,
...
const FB_PROGRAM = {
  ...
  description:
    'A classic 3-day full body routine. Hits every major muscle group using high-intensity, low-volume sets. Aim for 0-2 reps in reserve on every exercise.',
  level: 'beginner',
  goal: 'build_muscle',
  is_published: true,
  display_order: 3,
...
const STRENGTH_5X5_PROGRAM = {
  name: '5x5 Strength',
  description:
    'The classic beginner strength program. Simple, effective, and time-tested. Alternating A/B workouts 3x per week with linear progression.',
  level: 'beginner',
  goal: 'get_stronger',
  is_published: true,
  display_order: 4,
...
const STRENGTH_5X5_ROUTINES = [
  {
    name: 'Bench & Row',
    description:
      'Squat, Bench Press, Barbell Row. The foundation of building strength.',
    level: 'beginner',
    duration_minutes: 45,
    equipment: ['Barbell'],
    is_published: true,
...
const ARNOLD_PROGRAM = {
  name: 'Arnold Split',
  description:
    'The legendary 6-day split used by Arnold Schwarzenegger. High volume training for serious muscle growth. Chest/Back, Shoulders/Arms, Legs repeated twice per week.',
  level: 'advanced',
  goal: 'build_muscle',
  is_published: true,
  display_order: 5,
...
const ARNOLD_ROUTINES = [
  ...
  },
  {
    name: 'Shoulders & Arms',
    description:
      'Shoulder development and arm training for complete upper body aesthetics.',
    level: 'advanced',
    duration_minutes: 60,
    equipment: ['Barbell', 'Dumbbell', 'Cable'],
    is_published: true,
  ...
  },
  {
    name: 'Arnold Legs',
    description:
      'Complete leg development with heavy compounds and isolation work.',
    level: 'advanced',
    duration_minutes: 75,
    equipment: ['Barbell', 'Machine'],
    is_published: true,
...
const BW_PROGRAM = {
  ...
  description:
    'Build muscle and strength at home with zero equipment. A proven 3-day routine focusing on progressive bodyweight movements.',
  level: 'beginner',
  goal: 'build_muscle',
  is_published: true,
  display_order: 6,
...
const BW_ROUTINES = [
  ...
  },
  {
    ...
    exercises: [
      { name: 'Jump Squat', sets: 4, reps_min: 10, reps_max: 15 },
...
async function seed() {
  ...
  const { error: err2 } = await supabase
    ...
    .delete()
    .neq('program_id', '00000000-0000-0000-0000-000000000000')
