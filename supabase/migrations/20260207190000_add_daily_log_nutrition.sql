...
-- Adds day-level log container + meal-level macro entries for chat-based food logging.
...
comment on table public.daily_log_entries is 'One row per user per date for daily metrics, goals, and meal totals context.';
comment on table public.daily_log_meals is 'Individual meal logs with calories and macros that roll up to a daily total.';
