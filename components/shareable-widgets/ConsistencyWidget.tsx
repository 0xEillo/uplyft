...
export const ConsistencyWidget = React.forwardRef<View, ConsistencyWidgetProps>(
  (
    { workoutDates, variant, backgroundMode = 'light', userTag, displayName },
    ref,
  ) => {
    const datesSet =
      workoutDates instanceof Set ? workoutDates : new Set(workoutDates)
        ...
        <View style={styles.variantContainer}>
          ...
          <View style={styles.monthNamesRow}>
            {dayNames.map((name, i) => (
              <Text
                key={i}
                style={[styles.monthDayName, { color: subTextColor }]}
              >
                {name}
              </Text>
            ))}
    ...
    const renderYearlyView = () => {
      ...
      for (let month = 0; month < 12; month++) {
        ...
        months.push(
          <View key={month} style={styles.miniMonth}>
            <Text style={[styles.miniMonthName, { color: subTextColor }]}>
              {monthNames[month]}
            </Text>
      ...
      const yearWorkoutCount = [...datesSet].filter((d) =>
      ...
      const renderYearGraph = (year: number) => {
        ...
        return (
          <View key={year} style={styles.multiYearYearGroup}>
            <Text style={[styles.multiYearYearTitle, { color: subTextColor }]}>
              {year}
            </Text>
