...
interface ProgramCardProps {
  ...
  /** Parent-controlled: program has been saved to library (persistent). */
  isSaved?: boolean
...
export function ProgramCard({
  ...
  // Derive a single visual state from parent-controlled props so the "Saved"
  // pill persists across remounts (chat reloads). Local saving state is no
  // longer needed — the parent owns that too.
  const saveState: 'idle' | 'saving' | 'saved' = isSaved
    ? 'saved'
    : isSaving
    ? 'saving'
    : 'idle'
  ...
  const handleSavePress = () => {
    if (!onSaveProgram || saveState !== 'idle') return
    void onSaveProgram()
  }
      ...
      {onSaveProgram && (
        <View style={styles.actionsContainer}>
          <TouchableOpacity
            ...
            activeOpacity={saveState === 'idle' ? 0.85 : 1}
          ...
          >
            {saveState === 'saving' ? (
              ...
              <>
                ...
                <Text style={styles.primaryButtonText}>Saved to library</Text>
              </>
            ) : (
              <>
